...
import { X, CreditCard, ChevronLeft, ChevronRight, User, Phone } from 'lucide-react';
...
export default function CustomerLedgerModal({ customerId, customerName, onClose }: Props) {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn:  () => customersApi.getAccount(customerId, page).then(r => r.data),
    staleTime: 10_000,
  });
  ...
  const payMutation = useMutation({
    mutationFn: () => {
      ...
      return customersApi.addPayment(customerId, {
        amount,
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-sm mt-0.5">
                ...
                <span className="text-slate-400">
                  الرصيد:&nbsp;
                  {balance > 0
                    ? <span className="text-red-400 font-bold">{fmt(balance)} (دين)</span>
                    : balance < 0
                      ? <span className="text-green-400 font-bold">{fmt(Math.abs(balance))} (بذمتنا)</span>
        ...
        {showPay && (
          <div className="p-5 border-b border-slate-700 bg-slate-800/50 flex-shrink-0">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-white text-sm">تسجيل دفعة — {customer?.name ?? customerName}</h3>
            ...
            <form onSubmit={submitPay} className="grid grid-cols-2 gap-3">
              ...
              {payAmt > 0 && (
                <div className="col-span-2 rounded-lg bg-slate-700/60 border border-slate-600 px-3 py-2 text-xs text-slate-300 flex justify-between">
                  ...
                  <span>الرصيد بعد الدفعة: <strong className={newBal > 0 ? 'text-red-400' : newBal < 0 ? 'text-green-400' : 'text-slate-300'}>{fmt(Math.abs(newBal))} {newBal > 0 ? '(دين)' : newBal < 0 ? '(بذمتنا)' : ''}</strong></span>
                </div>
              )}
