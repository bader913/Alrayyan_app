        ...
        <div>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid var(--border)' }}>
            ...
            <div className="grid grid-cols-12 gap-2 px-4 py-2.5 text-xs font-black border-b"
              ...
              <div className="col-span-3">الكاشير</div>
              <div className="col-span-2">وقت الفتح</div>
              <div className="col-span-2">وقت الإغلاق</div>
              <div className="col-span-1 text-center">الافتتاح</div>
            ...
            {loadingHistory ? (
              ...
              (historyData ?? []).map((s, i) => {
                ...
                return (
                  <div key={s.id}
                    className="grid grid-cols-12 gap-2 px-4 py-3 items-center border-b text-sm"
                    style={{
                    ...
                    <div className="col-span-3">
                      <div className="font-bold truncate" style={{ color: 'var(--text-primary)' }}>{s.cashier_name}</div>
                      {s.terminal_name && <div className="text-xs" style={{ color: 'var(--text-muted)' }}>{s.terminal_name}</div>}
                    </div>
                    ...
                    <div className="col-span-1 text-center font-bold tabular-nums" style={{ color: 'var(--text-primary)' }}>
                      {fmtUSD(s.opening_balance)}
                    </div>
                    <div className="col-span-1 text-center font-black tabular-nums text-xs" style={{ color: diffColor }}>
                      {s.status === 'open' ? '—' : (Math.abs(diff) < 0.01 ? '✓' : (diff > 0 ? `+${fmtUSD(diff)}` : fmtUSD(diff)))}
                    </div>
