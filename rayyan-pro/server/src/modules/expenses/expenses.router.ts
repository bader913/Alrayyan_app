...
export async function expensesRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      await auditLog({ action: 'CREATE', entity: 'expense', entityId: (row as any).id, userId, details: input });
  ...
  fastify.put(
    ...
    async (request, reply) => {
      ...
      await auditLog({ action: 'UPDATE', entity: 'expense', entityId: parseInt(id), userId, details: input });
      return { expense: row };
    },
  );
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      ...
      await auditLog({ action: 'DELETE', entity: 'expense', entityId: parseInt(id), userId, details: {} });
      return { success: true };
    },
  );
}
