...
export class ProductsService {
  ...
  async listProducts(filters: ProductFilters) {
    ...
    const result = await pool.query<ProductRow>(
      `${PRODUCT_SELECT} ${where} ORDER BY p.name ASC LIMIT $${idx} OFFSET $${idx + 1}`,
  ...
  async getProductsByBarcode(barcode: string): Promise<ProductRow[]> {
    const result = await pool.query<ProductRow>(
      `${PRODUCT_SELECT} WHERE p.barcode = $1 AND p.is_active = TRUE ORDER BY p.name ASC`,
      [barcode]
    );
    return result.rows;
  }
  ...
  async createProduct(data: {
    ...
    await withTransaction(async (client) => {
      ...
      // Record initial stock if > 0 (always via stockMovements)
  ...
  async adjustStock(
    id: number,
    options: { new_quantity: number; note?: string; created_by: number }
  ): Promise<ProductRow> {
    await withTransaction(async (client) => {
      ...
      const current = parseFloat(product.rows[0].stock_quantity);
